...
const ErrorFallback = ({ error, onReload }) => (
  <div className="min-h-screen flex items-center justify-center p-8 bg-background">
    <div className="bg-surface rounded-2xl p-12 max-w-xl w-full text-center border border-border shadow-2xl">
      ...
      <details className="text-left bg-gray-50 rounded-lg p-4 mb-6 max-h-52 overflow-y-auto">
        <summary className="cursor-pointer text-text-secondary text-sm mb-2">Error details</summary>
        <pre className="text-xs text-red-500 whitespace-pre-wrap break-words">{error?.stack}</pre>
      </details>
      <button
        onClick={onReload}
        className="bg-primary text-white px-8 py-3 border-none rounded-lg font-semibold text-base cursor-pointer"
      >
        Reload Page
      </button>
    </div>
