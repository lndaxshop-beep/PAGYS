import{genAI as m,MODEL as f}from"./config-DRsAIlFP.js";import{getWordCountPreset as W}from"./config-DRsAIlFP.js";import{c as E,b as y}from"./sourceExtractor-B03JQ0Yd.js";import{e as X,g as q}from"./sourceExtractor-B03JQ0Yd.js";import{r as S}from"./instruments-BqSp0zGD.js";import{a as J,g as z,b as Q,c as j,d as Z,e as _,f as ee,h as te}from"./instruments-BqSp0zGD.js";const I=`IMPORTANT TABLE RULES:
...
Return ONLY the polished text. No explanations, no annotations.`,b=await s.generateContent(h),T=E(b.response.text());T&&T.trim().length>50&&(g=T)}catch(s){console.warn("[humaniseContent] Polish pass failed, using humanised output:",s.message)}return g}catch(n){throw console.error("Error humanising:",n),n}},x=async(e,t,n=null,r="ai-only")=>{try{const a=m.getGenerativeModel({model:f,tools:[{googleSearch:{}}]}),o=t==="apa"?"APA 7th edition: Author, A. A. (Year). Title of work. Source/Publisher. DOI or URL if available.":t==="mla"?"MLA 9th edition: Author Last, First. Title of Work. Publisher, Year.":"Chicago: Author Last, First. Year. Title of Work. Publisher.";let c="";n?.length>0&&(r==="user-only"||r==="combine")&&(c=`
...
List ALL variables with their full academic names. Use only the format above, no JSON.`;return(await t.generateContent(n)).response.text().trim()||null}catch(t){return console.error("Error:",t),null}},U=async e=>{try{const t=m.getGenerativeModel({model:f}),n=`Generate a theoretical framework description for a thesis.
...
List the key methodological steps in order. Use plain text, no diagrams.`;return(await t.generateContent(n)).response.text().trim()||null}catch(t){return console.error("Error:",t),null}},k=async(e,t,n)=>{try{const r=m.getGenerativeModel({model:f}),a=n?`
...
Return a markdown table with 4-6 rows of realistic data based on the research findings provided. Use proper column headers and realistic values.`;return(await r.generateContent(o)).response.text().trim()}catch(r){return console.error("Error:",r),null}},M=async(e,t,n,r)=>{try{const a=m.getGenerativeModel({model:f}),o=r?`
...
{"proposal":[{"question":"...","answer":"..."}],"chapter1":[{"question":"...","answer":"..."}]}`,u=(await t.generateContent(o)).response.text().match(/\{[\s\S]*\}/);if(u)try{return JSON.parse(u[0])}catch{}return null}catch(t){return console.error("Error generating defence questions:",t),null}},G=(e,t)=>!e||e.length===0?"":[...new Set(e)].sort().map(r=>{const a=r.split(/[, ]+/),o=a[0]||"Author",c=a[1]||"n.d.";switch(t){case"apa":return`${o}. (${c}). Title of the work. Publisher.`;case"mla":return`${o}. Title of the Work. Publisher, ${c}.`;case"chicago":return`${o}. ${c}. Title of the Work. Publisher.`;case"harvard":return`${o} (${c}). Title of the work. Publisher.`;default:return`${o} (${c})`}}).join(`
`),H=async(e,t)=>{try{const n=m.getGenerativeModel({model:f}),r=e.substring(0,15e3),a=`Extract field-specific abbreviations from this thesis content. Only include abbreviations that are specialized technical terms relevant to the thesis topic or academic field.
...
- Return ONLY the JSON array, no other text`,o=await n.generateContent(a);return y(o.response.text())||[]}catch(n){return console.error("Error extracting abbreviations:",n),[]}},Y=async(e,t)=>{try{const n=m.getGenerativeModel({model:f});let r="";Object.entries(t||{}).forEach(([u,i])=>{!i||typeof i!="object"||(r+=`
--- ${u} ---
...
Use formal academic language in a single cohesive paragraph. Do not include headings, labels, or bracketed instructions. Return ONLY the abstract text.`;return(await n.generateContent(o)).response.text().trim()||null}catch(n){return console.error("Error generating abstract:",n),null}};export{J as analyzeTranscriptText,N as applyFeedbackToContent,H as extractAbbreviations,X as extractPaperMetadata,G as formatReferences,Y as generateAbstract,v as generateAcademicContent,z as generateCaseStudyProtocol,M as generateChartData,$ as generateConceptualFramework,k as generateDataTable,P as generateDefenceQuestions,Q as generateDocumentAnalysisTemplate,j as generateFocusGroupProtocol,Z as generateInterviewGuide,q as generateLiteratureMatrix,_ as generateObservationChecklist,ee as generateQuestionnaire,x as generateReferences,F as generateResearchDesignFlowchart,te as generateSampleData,C as generateSubtopics,U as generateTheoreticalFramework,W as getWordCountPreset,L as humaniseContent,S as recommendLiteratureReviewType,O as selfReviewContent};
